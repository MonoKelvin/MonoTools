...
- **Testing**: All test code (Vitest + cargo) lives under `<repo>/tests/`. See [tests/README.md](./tests/README.md). Sub `tests/rust/` for cargo integration (loaded via `[[test]] path = "../tests/run.rs"` from `src-tauri/Cargo.toml`) and `tests/ui/` for Vitest. Vitest config: `vitest.config.ts` with happy-dom + `@/` alias.
...
- **Command Bus**: `src/commands/` — `commandRegistry.execute(id)` is the single dispatch point for all UI behavior (keyboard, context menu, tray menu). Built-in command builders in `src/commands/builtins/` (search/system). See [docs/COMMANDS.md](./docs/COMMANDS.md).
