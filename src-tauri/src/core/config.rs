...
pub mod icon {
    ...
    pub mod autocrop {
        ...
        /// 内容占比低于此值 → 裁剪并放大.
        /// 0.45: 常规 exe 图标内容通常占 55-85%, 不受影响;
        /// UWP 磁贴图标只有 35-55% 有内容 (周围透明 padding + 小 glyph),
        /// 降低阈值到 0.45 确保它们被裁剪放大, 让磁贴在列表中更清晰可见.
        /// 0.35 原值过高导致大多数 UWP 磁贴不触发裁剪.
        pub const MIN_AREA_RATIO: f32 = 0.45;
