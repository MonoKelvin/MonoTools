...
describe('useAppIcon — 监控 trace', () => {
    ...
    beforeEach(async () => {
        const { resetIconTrace } = await import('@/ui/widgets/appicon/iconLog')
        resetIconTrace()
    })
    ...
    it('cache 命中会记一次 level=cache', async () => {
        ...
        // 第二次命中 cache
        await loadIcon(result)
        const { useIconLog } = await import('@/ui/widgets/appicon/iconLog')
    ...
    it('resetTrace 后 counts 全部归零', async () => {
        ...
        const { useIconLog, resetIconTrace } = await import('@/ui/widgets/appicon/iconLog')
